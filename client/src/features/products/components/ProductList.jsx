...
export const ProductList = () => {
    ...
    const handleBrandFilters = (e) => {
        setFilters((prevFilters) => {
            const brandFilters = new Set(prevFilters.brand || []); // Ensure it's always an array
            
            if (e.target.checked) {
                brandFilters.add(e.target.value);
            } else {
                brandFilters.delete(e.target.value);
            }
    
            return { ...prevFilters, brand: Array.from(brandFilters) };
        });
    };

    const handleCategoryFilters=(e)=>{
        const filterSet=new Set(filters.category)
        console.log(filters.category,"log")
  ...
  return (
...

    {
        ...
        :
        <>
        <motion.div style={{position:"fixed",backgroundColor:"white",height:"100vh",padding:'1rem',overflowY:"scroll",width:is500?"100vw":"30rem",zIndex:500}}  variants={{show:{left:0},hide:{left:-600}}} initial={'hide'} transition={{ease:"easeInOut",duration:.7,type:"spring"}} animate={isProductFilterOpen===true?"show":"hide"}>

            {/* fitlers section */}
            <Stack mb={'5rem'}   sx={{scrollBehavior:"smooth",overflowY:"hidden"}}>
                                ...
                                <FormGroup onChange={handleBrandFilters}>
                                    {
                                        brands?.map((brand)=>(
                                            ...
                                            control={
                                                <Checkbox 
                                                    checked={filters.brand?.includes(brand._id) || false} // Ensure it reflects the state
